...
return new class extends Migration {
    ...
    public function up(): void
    {
        Schema::create("roles", function (Blueprint $table) {
            ...
            $table->string("guard_name")->default("web"); // For spatie/laravel-permission compatibility
            $table->timestamps();
        });
        ...
        Schema::create("rotation_assignments", function (Blueprint $table) {
            ...
            $table
                ->foreignId("user_id")
                ->constrained("users")
                ->cascadeOnDelete(); // Koas
            $table
                ->foreignId("rotation_id")
                ->constrained("rotations")
                ->cascadeOnDelete();
            $table
                ->foreignId("academic_period_id")
                ->constrained("academic_periods")
                ->cascadeOnDelete();
            $table
                ->foreignId("supervisor_id")
                ->constrained("users")
                ->cascadeOnDelete(); // Dosen
            $table->date("start_date");
            $table->date("end_date");
            $table->timestamps();
        });
        ...
        Schema::create("patient_logs", function (Blueprint $table) {
            ...
            $table
                ->enum("status", ["draft", "submitted", "revised", "approved"])
                ->default("draft");
        ...
        Schema::create("procedure_validations", function (Blueprint $table) {
            $table->id();
            $table
                ->foreignId("patient_log_id")
                ->nullable()
                ->constrained("patient_logs")
                ->nullOnDelete();
            $table
